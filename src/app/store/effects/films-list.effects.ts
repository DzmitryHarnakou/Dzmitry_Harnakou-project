...
import { apiUrl, getMovieSearchLink } from '../../services/api.config';
...
export class FilmsListEffects {
  ...
  private getSearchResults(action:any) {
      ...
      this.n = 1;
      var link:string = getMovieSearchLink(this.formState.adult, this.formState.title, this.formState.overview, this.n);
  ...
  switchMap(() => from(this.getFilmList ()).pipe(map((moviePageData)=> new filmListActions.LoadFilmListsSuccess(this.sort(moviePageData))),
  ...
  switchMap(() => from(this.loadNextPage ()).pipe(map((moviePageData)=> new filmListActions.LoadNextPageSuccess(this.sort(moviePageData))),
  ...
  switchMap((payload) => from(this.getSearchResults(payload)).pipe(map((searchData)=> new filmListActions.SearchFilmsSuccess(searchData)),
  catchError(() => of(new filmListActions.SearchFilmsError("Can't load((("))))))

  @Effect ()
  public getNextSearchResults$ = this.actions$.pipe(ofType(filmListActions.FilmListActionTypes.LoadNextSearchPage),
  switchMap(() => from(this.getNextSearchResults()).pipe(map((searchData)=> new filmListActions.LoadNextSearchPageSuccess(searchData)),
  catchError(() => of(new filmListActions.LoadNextSearchPageError("Can't load((("))))))
}
